...
def main_train(img1, img2, img3, newsize=1024):
    im1 = Imager(img1) # THIS IS OUR BASE, tunnel this shit doge
    im2 = Imager(img2) # MIDDLE, transparent gradency lag - mikkelololoololoololololooooooolllll
    im3 = Imager(img3) # SUPERTWERK - STIAN XD - litt av bilde og fade inn et eller annet sted. dritfe77 $vai
...
def ptest2(fid1='images/einstein.jpeg',outfid='images/tunnel.jpeg',myvariable=3,newsize=250,scale=0.8):
    im1 = Imager(fid1);
    im1 = im1.resize(newsize,newsize);
#    im1.filter(ImageFilter.BLUR)
    im2 = im1.tunnel(levels=myvariable,scale=scale, threshold=10*9)
#    im1.color()
#    im1.contrast()
#    im1.sharpen()
#    im1.solarize(threshold=64)
    im2.display()
